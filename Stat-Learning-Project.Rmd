...
We have to add names for our variables.
```{r}
...
Let's look at a table of values for the response variable
```{r}
table(spamdata$spam)
```
...
As FYI, the tuning model took about 10 minutes to run.  Our regular Random Forest ran with mtry = sqrt(ncol(training_data -1)) = sqrt(57) = 7.55, and that resulted in an accuracy of approximately 0.9494.  With our parameter tuning, we see that mtry = 15 is the best choice with a resulting accuracy of 0.9528.
...
##### QDA
I don't think QDA can work for this data.  We have a rank deficiency issue.  Rank deficiency in this context says there is insufficient information contained in your data to estimate the model you desire.
```{r}
...
## Use the nfactors function
The "nfactors" function will help us determine how many factors to maintain
```{r}
spamdata %>% 
  dplyr::select(-spam) %>%
  na.omit() %>% 
...
The "nfactors" function suggests between 3 and 20 factors.  Let's try parallel analysis.
## Parallel Analysis
```{r}
spamdata %>%
  dplyr::select(-spam) %>%
...
Parallel analysis suggests between 3 and 17 factors.  Let's see if a correlation plot helps.
...
The plot seems to suggest a lower number of factors.  Let's run factor analysis using different numbers of factors between 3 and 20.
### Run EFA with 3 factors and oblique rotation
```{r}
...
#### Check how the 2 factor scores correlate
```{r}
obliqueRotation_3f$score.cor
```

I am not sure how we can use these factors in our analysis.  We might be able to create some scores that we can use to predict spam, but we will have to talk about this further.